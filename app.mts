...
async function fetchProductIDsByName(productName: string): Promise<ProductEdge[]> {
    const query = `
    {
...
async function fetchVariantsByProductID(productID: string): Promise<Product | null> {
    const query = `
    {
...
function displayProductVariants(products: Product[]) {
    const variantsList: { title: string; variant: string; price: number }[] = [];
